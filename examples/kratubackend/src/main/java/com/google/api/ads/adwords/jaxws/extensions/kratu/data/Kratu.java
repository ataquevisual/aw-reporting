...
@Entity
@Table(name = "AW_Kratu")
@org.hibernate.annotations.Table(appliesTo = "AW_Kratu", indexes = { @org.hibernate.annotations.Index(name = "ID-DAY", columnNames = {
    "EXTERNAL_CUSTOMER_ID", "DAY" }) })
public class Kratu implements MongoEntity {
	...
	// Creates a Kratu, summarizing for a DateRange and for the same
	// externalCustomerId.
	// Each item on the list is one Day.
	protected static Kratu createKratuSummary(List<Kratu> dailyKratus,
	    Date startDate, Date endDate) {

		Kratu summarizedKratu = new Kratu();

		BigDecimal daysInRange = new BigDecimal(Days.daysBetween(
		    new DateTime(startDate.getTime()), new DateTime(endDate.getTime()))
		    .getDays()).add(BigDecimal.ONE);
		...
		for (Kratu dailyKratu : dailyKratus) {
			...
			summarizedKratu.spend = summarizedKratu.spend.setScale(2,
			...
			summarizedKratu.elegibleImpressionsSearch = summarizedKratu.elegibleImpressionsSearch
			    .add(dailyKratu.elegibleImpressionsSearch);
			...
			summarizedKratu.ctrSearch = summarizedKratu.ctrSearch
			    .add(dailyKratu.ctrSearch
			        .divide(daysInRange, 2, RoundingMode.HALF_UP));
			...
			summarizedKratu.averageCpmSearch = summarizedKratu.averageCpmSearch
			    .add(dailyKratu.averageCpmSearch.divide(daysInRange, 2,
			        RoundingMode.HALF_UP));
			summarizedKratu.averagePositionSearch = summarizedKratu.averagePositionSearch
			    .add(dailyKratu.averagePositionSearch.divide(daysInRange, 2,
			        RoundingMode.HALF_UP));
			...
			summarizedKratu.elegibleImpressionsDisplay = summarizedKratu.elegibleImpressionsDisplay
			    .add(dailyKratu.elegibleImpressionsDisplay);
			...
			summarizedKratu.numberOfNegativeActiveKeywords = summarizedKratu.numberOfNegativeActiveKeywords
			    .add(dailyKratu.numberOfNegativeActiveKeywords.divide(daysInRange, 2,
			        RoundingMode.HALF_UP));
	...
	/*
	 * Retrieves 7 reports types per account and creates a Kratu data object for
	 * each day.
	 */
	public static Kratu createDailyKratuFromDB(StorageHelper storageHelper,
	    Long topAccountId, Account account, Date day) {

		Long accountId = account.getExternalCustomerId();

		List<ReportAccount> reportAccountList = storageHelper.getReportByAccountId(
		    ReportAccount.class, accountId, day, day);
		...
		if (reportAccountList.size() == 0) {
			...
			List<ReportCampaign> reportCampaignList = storageHelper
			    .getReportByAccountId(ReportCampaign.class, accountId, day, day);
			List<ReportAdGroup> reportAdGroupList = storageHelper
			    .getReportByAccountId(ReportAdGroup.class, accountId, day, day);
			List<ReportAd> reportAdList = storageHelper.getReportByAccountId(
			    ReportAd.class, accountId, day, day);
			List<ReportKeyword> reportKeywordList = storageHelper
			    .getReportByAccountId(ReportKeyword.class, accountId, day, day);
			List<ReportAdExtension> reportAdExtensionsList = storageHelper
			    .getReportByAccountId(ReportAdExtension.class, accountId, day, day);

			List<ReportCampaignNegativeKeyword> reportCampaignNegativeKeywordList = storageHelper
			    .getReportCampaignNegativeKeywordByAccountAndEndDateInRange(
			        accountId, day, day);
			...
			for (ReportAccount reportAccount : reportAccountList) {
				...
				if (reportAccount.getAdNetwork() != null
					...
					BigDecimal searchImpressions = reportAccount
					    .getSearchImpressionShareBigDecimal().divide(BIGDECIMAL_100);
					if (searchImpressions.intValue() != 0) {
						kratu.elegibleImpressionsSearch = new BigDecimal(
						    kratu.impressionsSearch).divide(searchImpressions, 2,
						    RoundingMode.HALF_UP);
					} else {
						kratu.elegibleImpressionsSearch = BigDecimal.ZERO;
					}

					kratu.lostImpressionsDueToBudgetSearch = reportAccount
					    .getSearchLostISBudgetBigDecimal().divide(BIGDECIMAL_100)
					    .multiply(kratu.elegibleImpressionsSearch);
					kratu.lostImpressionsDueToBidAdRankSearch = reportAccount
					    .getSearchLostISRankBigDecimal().divide(BIGDECIMAL_100)
					    .multiply(kratu.elegibleImpressionsSearch);
					...
					kratu.averagePositionSearch = reportAccount
					    .getAvgPositionBigDecimal();
				}
				// Display
				if (reportAccount.getAdNetwork() != null
					...
					kratu.elegibleImpressionsDisplay = new BigDecimal(
					    kratu.impressionsDisplay).divide(reportAccount
					    .getContentImpressionShareBigDecimal().divide(BIGDECIMAL_100), 2,
					    RoundingMode.HALF_UP);

					kratu.lostImpressionsDueToBudgetDisplay = reportAccount
					    .getContentLostISBudgetBigDecimal().divide(BIGDECIMAL_100)
					    .multiply(kratu.elegibleImpressionsDisplay);
					kratu.lostImpressionsDueToBidAdRankDisplay = reportAccount
					    .getContentLostISRankBigDecimal().divide(BIGDECIMAL_100)
					    .multiply(kratu.elegibleImpressionsDisplay);
					...
					kratu.averagePositionDisplay = reportAccount
					    .getAvgPositionBigDecimal();
				}
			}
			...
			for (ReportCampaign reportCampaign : reportCampaignList) {
				if (reportCampaign.getStatus().equals(ACTIVE)) {
					kratu.numberOfActiveCampaigns = kratu.numberOfActiveCampaigns
					    .add(BigDecimal.ONE);
				}

				if (reportCampaign.getBudgetBigDecimal() != null) {
					kratu.sumBudget = kratu.sumBudget.add(reportCampaign
					    .getBudgetBigDecimal());
				}
			}
			...
			for (ReportAdGroup reportAdGroup : reportAdGroupList) {
				if (reportAdGroup.getStatus().equals(ENABLE)) {
					kratu.numberOfActiveAdGroups = kratu.numberOfActiveAdGroups
					    .add(BigDecimal.ONE);
				}
			}
			...
			for (ReportAd reportAd : reportAdList) {
				...
				if (reportAd.getCreativeApprovalStatus().equals(DISAPPROVED)) {
					kratu.numberOfDisapprovedAds = kratu.numberOfDisapprovedAds
					    .add(BigDecimal.ONE);
				}
			}
			reportAdList = null;
			...
			for (Integer activeAds : activeAdsPerAdGroup.values()) {
				if (activeAds == 1) {
					kratu.numberOfAdgroupsWithoneActiveAd = kratu.numberOfAdgroupsWithoneActiveAd
					    .add(BigDecimal.ONE);
				}
				if (activeAds == 2) {
					kratu.numberOfAdgroupsWithTwoActiveAds = kratu.numberOfAdgroupsWithTwoActiveAds
					    .add(BigDecimal.ONE);
				}
			}
			...
			for (ReportKeyword reportKeyword : reportKeywordList) {
				if (reportKeyword.getStatus().equals(ENABLE)) {
					if (!reportKeyword.isNegative()) {
						kratu.numberOfPositiveActiveKeywords = kratu.numberOfPositiveActiveKeywords
						    .add(BigDecimal.ONE);

						// numberOfActiveGoodQualityScoreKeywords: QS > 7
						if (reportKeyword.getQualityScoreAsBigDecimal().compareTo(
						    BigDecimal.valueOf(7)) == 1) {
							kratu.numberOfActiveGoodQualityScoreKeywords = kratu.numberOfActiveGoodQualityScoreKeywords
							    .add(BigDecimal.ONE);
						}
						// numberOfActiveAverageQualityScoreKeywords: QS>4 && <=7
						if (reportKeyword.getQualityScoreAsBigDecimal().compareTo(
						    BigDecimal.valueOf(4)) == 1
						    && reportKeyword.getQualityScoreAsBigDecimal().compareTo(
						        BigDecimal.valueOf(7)) < 1) {
							kratu.numberOfActiveAverageQualityScoreKeywords = kratu.numberOfActiveAverageQualityScoreKeywords
							    .add(BigDecimal.ONE);
						}
						// numberOfActivePoorQualityScoreKeywords: QS <= 4
						if (reportKeyword.getQualityScoreAsBigDecimal().compareTo(
						    BigDecimal.valueOf(4)) < 1) {
							kratu.numberOfActivePoorQualityScoreKeywords = kratu.numberOfActivePoorQualityScoreKeywords
							    .add(BigDecimal.ONE);
						}

						if (reportKeyword.getKeywordMatchType().equals(BROAD))
							kratu.numberOfActiveBroadMatchingKeywords = kratu.numberOfActiveBroadMatchingKeywords
							    .add(BigDecimal.ONE);

						if (reportKeyword.getKeywordMatchType().equals(PHRASE))
							kratu.numberOfActivePhraseMatchingKeywords = kratu.numberOfActivePhraseMatchingKeywords
							    .add(BigDecimal.ONE);

						if (reportKeyword.getKeywordMatchType().equals(EXACT))
							kratu.numberOfActiveExactMatchingKeywords = kratu.numberOfActiveExactMatchingKeywords
							    .add(BigDecimal.ONE);

						sumImpressions += reportKeyword.getImpressions();
						totalPositions = totalPositions.add(reportKeyword
						    .getAvgPositionBigDecimal().multiply(
						        BigDecimal.valueOf(reportKeyword.getImpressions())));
						totalWeight = totalWeight.add(reportKeyword
						    .getQualityScoreAsBigDecimal().multiply(
						        BigDecimal.valueOf(reportKeyword.getImpressions())));

					} else {
						kratu.numberOfNegativeActiveKeywords = kratu.numberOfNegativeActiveKeywords
						    .add(BigDecimal.ONE);
						kratu.numberOfAdGroupNegativeActiveKeywords = kratu.numberOfAdGroupNegativeActiveKeywords
						    .add(BigDecimal.ONE);
					}
				}
			}
			if (sumImpressions > 0) {
				kratu.weightedAverageKeywordPosition = totalPositions.divide(
				    BigDecimal.valueOf(sumImpressions), 2, RoundingMode.HALF_UP);
				kratu.weightedAverageQualityScore = totalWeight.divide(
				    BigDecimal.valueOf(sumImpressions), 2, RoundingMode.HALF_UP);
			}
			...
			for (ReportAdExtension reportAdExtension : reportAdExtensionsList) {
				if (reportAdExtension.getStatus().equals(ACTIVE)) {
					if (reportAdExtension.getAdExtensionType().equals(LOCATION_EXTENSION)) {
						kratu.numberOfCampaignsWithLocationExtensionEnabled = kratu.numberOfCampaignsWithLocationExtensionEnabled
						    .add(BigDecimal.ONE);
					}
					if (reportAdExtension.getAdExtensionType().equals(MOBILE_EXTENSION)) {
						kratu.numberOfCampaignsWithCallExtensionEnabled = kratu.numberOfCampaignsWithCallExtensionEnabled
						    .add(BigDecimal.ONE);
					}
					if (reportAdExtension.getAdExtensionType().equals(
					    SITE_LINKS_EXTENSION)) {
						kratu.numberOfCampaignsWithSiteLinksEnabled = kratu.numberOfCampaignsWithSiteLinksEnabled
						    .add(BigDecimal.ONE);
					}
				}
			}
			...
			kratu.numberOfNegativeActiveKeywords = kratu.numberOfNegativeActiveKeywords
			    .add(new BigDecimal(reportCampaignNegativeKeywordList.size()));
			kratu.numberOfCampaignNegativeActiveKeywords = kratu.numberOfCampaignNegativeActiveKeywords
			    .add(new BigDecimal(reportCampaignNegativeKeywordList.size()));
